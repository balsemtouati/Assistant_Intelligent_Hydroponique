"""Scraper pour la catégorie 'cultures'

Objectif: depuis https://www.croquepousse.com/cultures/ récupérer chaque article (bouton / lien "Lire la suite"),
extraire métadonnées + sections structurées (h2/h3/h4) + listes / tableaux / images / liens.

Sorties:
  - JSONL (1 ligne / article) : cultures_data/cultures_articles.jsonl
  - (option) JSON consolidé des sections: --sections-json cultures_sections.json

Usage rapide:
  python scrap_cultures.py --max-pages 1 --limit 2
  python scrap_cultures.py --sections-json cultures_sections.json

Notes:
  - Section d'intro (heading=None) si texte avant le premier h2/h3/h4.
  - Section splitting amélioré: on n'accumule que des tags de type bloc pour réduire la duplication.
"""
...
BASE_URL = "https://www.croquepousse.com/cultures/"
DOMAIN = urlparse(BASE_URL).netloc
UA = "HydroCareResearchBot/1.0 (+contact: ton_email@example.com)"

BLOCK_TAGS = {"p","ul","ol","table","figure","blockquote","pre","img","div","section"}
...
def build_session(retries=3, backoff=0.8):
    s = requests.Session()
...
def split_sections(content: Tag) -> List[Section]:
    ...
    def flush():
        ...
        lists = collect_lists(container) if container else []  # type: ignore[arg-type]
        tables = collect_tables(container) if container else []  # type: ignore[arg-type]
        images = collect_images(container) if container else []  # type: ignore[arg-type]
        txt_join = "\n".join(text_parts)
    ...
    for el in content.descendants:
        ...
        if el.name in heading_tags:
            ...
            cur_level = int(el.name[1]) if el.name and el.name[1].isdigit() else None
            continue
        # On n'empile que des blocs (réduit duplications)
        # heading déjà géré plus haut
        # Ajout blocs
        if el.name in BLOCK_TAGS:
            # Ne pas dupliquer un bloc déjà sous un autre bloc présent (simple heuristique: éviter d'ajouter les parents génériques)
...
def parse_article(session: requests.Session, url: str, delay: float) -> Article:
    ...
    for img in content.select("img") if content else []:
        ...
        if not src or src in seen:
            continue
        images_global.append({"src": src, "alt": _to_str(img.get("alt")) or ""})
        seen.add(src)
...
def crawl(args):
    ...
    jsonl_path = os.path.join(args.output_dir, 'cultures_articles.jsonl')
    array_accum: Optional[List[dict]] = [] if args.json_file else None  # accumulation pour export unique JSON
...
def build_parser():
    p = argparse.ArgumentParser(description="Scraper cultures (articles détaillés)")
    p.add_argument('--base-url', default=BASE_URL)
    p.add_argument('--max-pages', type=int, default=50)
    p.add_argument('--delay', type=float, default=1.0)
    p.add_argument('--retries', type=int, default=3)
    p.add_argument('--output-dir', default='cultures_data')
    p.add_argument('--limit', type=int, help='Limiter nombre total articles')
    p.add_argument('--sections-json', help='Exporter JSON condensé des sections')
    p.add_argument('--json-file', help="Exporter aussi toutes les données dans un seul fichier JSON (ex: cutltures.json)")
    return p
