"""Scraper pour la catégorie 'mesures'

URL de base: https://www.croquepousse.com/mesures/
Objectif: récupérer tous les articles (boutons "Lire la suite"), extraire:
 - Métadonnées (titre, auteur, date, meta description, canonical)
 - Catégories, tags
 - Sections structurées (h2/h3/h4) + intro
 - Listes, tableaux, images par section
 - Liens internes / externes / affiliation (amazon)
 - Hash contenu & nombre de mots

Sorties:
  - JSONL: mesures_data/mesures_articles.jsonl
  - Option JSON array (--json-file) ex: mesures_all.json
  - Option sections condensées (--sections-json)

Usage:
  python scrap_mesures.py --max-pages 1 --limit 2
  python scrap_mesures.py --json-file mesures_all.json
  python scrap_mesures.py --sections-json mesures_sections.json --json-file mesures_all.json
"""
...
BASE_URL = "https://www.croquepousse.com/mesures/"
...
def crawl(args):
    ...
    jsonl_path = os.path.join(args.output_dir, 'mesures_articles.jsonl')
    # Toujours accumuler pour produire mesures_all.json automatiquement
    array_accum: List[dict] = []
    ...
    # Export JSON array par défaut (nom fixe mesures_all.json si non spécifié)
    out_name = 'mesures_all.json'
...
def build_parser():
    p = argparse.ArgumentParser(description="Scraper mesures (articles détaillés)")
    p.add_argument('--base-url', default=BASE_URL)
    p.add_argument('--max-pages', type=int, default=40)
    p.add_argument('--delay', type=float, default=1.0)
    p.add_argument('--retries', type=int, default=3)
    p.add_argument('--output-dir', default='mesures_data')
    ...
    # Option retirée: l'export mesures_all.json est maintenant automatique
    return p
